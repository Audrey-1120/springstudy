...
public class MyController1 {
   ...
   *  2) 주요 매개변수
   *    (1) HttpServletRequest request (doGet에서 사용했던 것!)
   *    (2) HttpServletResponse response (doGet에서 사용했던 것!)
   *    (3) HttpSession session 
   *    (4) 커멘드 객체 : 요청 파라미터를 받는 객체
   ...
   *    - 원래 redirect할때는 전달이 안되는데, 이것은 허용함. 
   */
  
  // value = "/"        : contextPath 요청을 의미한다.  http://localhost:8080/prj02
  // value = "/main.do" : contextPath 요청을 의미한다.  http://localhost:8080/prj02/main.do
  @RequestMapping(value={"/", "/main.do"}, method=RequestMethod.GET)
  public String welcome() {
    // 뷰리졸버 prefix : /WEB-INF/views/
    // 뷰리졸버 suffix : .jsp
    // 실제리턴        : /WEB-INF/views/index
    return "index";
  }
