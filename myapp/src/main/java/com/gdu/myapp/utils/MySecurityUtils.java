...
public class MySecurityUtils {
  ...
  public static String getSha256(String original) {
    ...
    try {
      MessageDigest digest = MessageDigest.getInstance("SHA-256"); //singleton으로 동작한다.
      digest.update(original.getBytes());
      byte[] bytes = digest.digest();
      for(int i = 0; i < bytes.length; i++) { // 2자리 16진수 - 2X
        // 만약에 앞자리 네자리가 다 0000이 되면 이건 0이다. 0은 표기를 하지 않기 때문에
  ...
  public static String getPreventXss(String original) {
    return original.replace("<", "&lt;").replace(">", "&gt");
  }
   ...
   */
  public static String getRandomString(int count, boolean letter, boolean number) {   // 인증 코드에 숫자를 포함?
    StringBuilder builder = new StringBuilder();
