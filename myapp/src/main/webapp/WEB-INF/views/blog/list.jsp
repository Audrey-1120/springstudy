...
const fnGetBlogList = () => {
	...
	$.ajax({
	 ...
	 success:(resData) => {  // resData = {"blogList": [], "totalPage": 10}
		  totalPage = resData.totalPage;  // totalPage 값 가져와서 갱신.
		  $.each(resData.blogList, (i, blog) => { // i : 인덱스, blog : 각 요소 가져오기
			  let str = '<div class="blog" data-blog-no="' + blog.blogNo + '" data-user-no="' + blog.user.userNo + '">';
...
const fnScrollHandler = () => {
  ...
  const fnInsertCount = () => {
	  let insertCount = '${insertCount}'; // 숫자지만 ''로 묶어서 넣는다.
	  console.log(insertCount);
	  if(insertCount !== '') {
		  if(insertCount === '1') {
			  alert('블로그가 등록되었습니다.');
		  } else {
			  alert('블로그가 등록되지 않았습니다.');
		  }
	  }
  }
